...
| [Veryfi Menu OCR API](https://www.veryfi.com/restaurant-menu-ocr-api/) | Commercial API — free trial, paid plans on request | OCR → NLP | `dish_name`, `dish_price`, `menu_section`, `dish_description` | SOC 2 Type II (advertised on their site). Mature infra. Output stops at a flat dish list — no aliases, no search terms, no specials lane. |
| [Klippa DocHorizon Menu OCR](https://klippa.com/en/ocr/data-fields/menu-cards) (now branded **Doxis**) | Commercial, custom quote | OCR → TXT → NLP | Dishes + categories + prices | Hosted in EU. Officially supports Latin-alphabet languages (English, Dutch, Norwegian, Danish, Swedish, Finnish, Italian, Portuguese, Spanish, German, French) per their FAQ — other languages "on request". Same output shape as Veryfi: flat dish list. |
